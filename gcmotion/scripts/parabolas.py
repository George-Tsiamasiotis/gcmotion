...
Simple script that calculates the RW,LW,MA parabolas diagram (constant
:math:`\mu` slices in the :math:`\theta`, :math:`P_{\zeta}`, E COM space).
"""
...
def calc_parabolas_tpb(
    ...
    **kwargs,
    ...
    In this script the values of the parabolas that correspond to the right
    and left boundary and to the magnetic axis, are calculated, along with
    the trapped passing boundary. CΑUTION: this script provides correct values
    only if there is no electric field.
    ...
    TPB_density : int, optional
        The points density of :math:`P_{\zeta}`s that will be used to
        calculate the trapped passing boundary. Defaults to 100.

    Returns
    -------
    Dict that contains the the x values x and x_TPB that were used to
    calculate the parabolas values and the trapped passing boundary values
    respectively, y_R,y_L,Y_MA the parabolas values for the right and left
    wall and magnetic axis parabolas, TPB_O, TPB_X the lower and upper
    branch(es) of the trapped passing boundary curve respectively,
    v_R, v_L, v_MA the location (points) of the minima (vertex) of each
    parabola.
    ...
    logger.info(
        f"""Upacked parabolas bfield values with Bmin={BminNU},
        Bmax={BmaxNU}, B0={B0NU}"""
    )

    # Because Pzetas given in Pzetalim are already divided (normalized)
    # by psip_wall
    x = PzetasNU

    # Currents are poloidally symmetrical --> independent of theta.
    # Calculate g.
    _, _, g_psipwNU = bfield.bigNU(psip_wallNU, 0)
    _, _, g0NU = bfield.bigNU(0.0, 0)

    logger.info(
        f"""Upacked parabolas g values with
         g(psip_wall)={g_psipwNU}, g(0)={g0NU}"""
    )
    ...
    y_R = (
        1
        / (2 * muB0NU)
        * (psip_wallNU * BminNU / g_psipwNU) ** 2
        * (x + 1) ** 2
        + BminNU / B0NU
    )
    ...
    y_L = (
        1
        / (2 * muB0NU)
        * (psip_wallNU * BmaxNU / g_psipwNU) ** 2
        * (x + 1) ** 2
        + BmaxNU / B0NU
    )
    ...
    logger.info(
        "Calculated parabolas y(x) values where y=E/muB0 and x=Pz/psip_wall"
    )
    ...
    if calc_TPB and not bfield.plain_name == "LAR":
        # If the equilibrium is not LAR we need to constrauct the energy
        #  bifurcation diagram with respect to Pzeta, which is the trapped
        #  passing boundary.

        # Create profiles that will be passed into bifurcation.
        # CAUTION: the Pzetas are in NUcanmom and not divided by psip_wallNU
        ...
        # We do not yet divide by B0muNU yet as the energies will be
        # needed later in NUJoule
        # Bifurcation returns deque() objects. We cast into lists.
        TPB_O = list(O_energies)
        TPB_X = list(X_energies)

        logger.info(
            f"""Ran bifurcation for parabolas TPB for {config.TPB_density}
        ...
        logger.info(
            f"""Calculated LAR TPB boundary with x axis limits:
              ({min(x_TPB)},{max(x_TPB)})"""
        )

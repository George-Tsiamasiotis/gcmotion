...
Fixed Points
============
Script/Function that calculates the fixed points of the GC Hamiltonian
for a given profile (magnetic field, electric field, qfactor, particle species
etc.)
"""
...
from gcmotion.scripts.fixed_points_bif.fp_ic_scan import (
    fp_ic_scan as ic_scanner,
)
...
def fixed_points(profile: Profile, **kwargs) -> dict:
    r"""
    Function that finds the fixed points of the GC Hamiltonian by numerically
    setting the time derivatives of the :math:`\theta` and :math:`\psi`
    variables equal to zero (numerical solvers) for a number of different
    initial conditions. White's equations are used.
    ...
    these fixed points. :math:`\psi` is calculated in "NUMagnetic_flux" units.
    ...
    logger.info(
        f"""Converting psi_lim from {config.psilim} 'NUpsi_wall' units to
    ...
    logger.info(
        f"""Converted psi_lim from 'NUpsi_wall' units to {psi_lim}
         'NUMagnetic_flux units'"""
    )
    ...
    logger.info(
        f"""Calculated fixed points {fixed_points} ['NUMagnetic_flux']
         in fixed_points script"""
    )
    ...
    logger.info(
        f"""Calculated {num_of_dfp} distinct fixed points
         {distinct_fixed_points} ['NUMagnetic_flux'] in fixed_points script"""
    )

    if config.fp_ic_info:
        initial_conditions_print = [
            [float(x), float(y)] for x, y in initial_conditions
        ]
        logger.info(f"\nInitial Conditions:{initial_conditions_print}\n")
        logger.info(
            f"""\nNumber of Initial Conditions:
             {len(initial_conditions_print)}\n"""
        )
    ...
    if config.fp_info and distinct_fixed_points.shape[0] <= 12:
        ...
        logger.info(
            f"""Number of Distinct Fixed Points:
             {distinct_fixed_points.shape[0]}\n"""
        )

    return {
        "num_of_dfp": num_of_dfp,
        "distinct_fixed_points": distinct_fixed_points,
        "initial_conditions": initial_conditions,
    }


def _distinctify(points: np.ndarray | list | deque, tol: float) -> np.ndarray:
    r"""
    Simple function that determines which elements [,] of a list of lists
    of len 2 [[,],[,],[,]...] can be considered distinct from one another.
    In this project's context, it is used to determine which points [x,y]
    can be considered distinct.

    Parameters
    ----------
    points : np.ndarray | list | deque
        Iterable (np.ndarray, list, deque) that contains sublists that are
        to be examined for uniquness.
    tol : list
        If two sublists have both elements that are less than tol
        (tolerance) apart, they are considered idenical.
    ...
    def are_considered_equal(sublist1, sublist2, tol=tol):
        return (
            abs(sublist1[0] - sublist2[0]) <= tol
            and abs(sublist1[1] - sublist2[1]) <= tol
        )
...
def _set_up_fixed_points(
    ...
    """
    Function that sets up some parameters of the fixed points' system,
    numerical solvers, initial conditions.
    """

    # CAUTION: Here psi lim has already been denormalized from psi_wall
    # and converted to NUMagnetic_flux.
    # This happened in fixed_points()
    psi_min = psilim[0]
    psi_max = psilim[1]
    ...
    if random_init_cond:
        ...
        initial_conditions = [
            [theta_init, psi_init]
            for theta_init, psi_init in product(theta_ic, psi_ic)
        ]
        ...
        if not initial_conditions:
            logger.info("No initial conditions were found from fp_ic_scan")
    ...
    empty_fixed_points = np.empty(
        (len(initial_conditions), len(initial_conditions[0]))
    )

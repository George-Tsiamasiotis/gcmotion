...
Script that finds fixed points candidates to be passed into the
:py:func:`single_fixed_point` function's numerical solver as initial
conditions (fp_is_scan --> fixed points initial conditions scan)
"""
...
def fp_ic_scan(
    ...
    r"""
    Function that finds fixed points candidates to used as initial conditions
    for the numerical solver in :py:func:`single_fixed_point`, by scanning a
    :math:`\theta`, :math:`\psi` 2D grid and storing the values
    (:math:`\theta`,:math:`\psi`) for which the time derivatives of
    ...
    theta_grid_density : int, optional
        Density of the :math:`\theta`, :math:`\psi` 2D grid with respect to
        the :math:`\theta` variable. Defaults to 500.
    psi_grid_density : int, optional
        Density of the :math:`\theta`, :math:`\psi` 2D grid with respect to
        the :math:`\psi` variable. Defaults to 101.
    theta_lim : list, optional
        Limits of the of the :math:`\theta`, :math:`\psi` 2D grid with
        respect to the :math:`\theta` variable.
        Defaults to [-:math:`\pi`, :math:`\pi`].
    psi_lim : list, optional
        Limits of the of the :math:`\theta`, :math:`\psi` 2D grid with respect
        to the :math:`\psi` variable. Defaults to [0.01 , 1.8]. CAUTION:
        The limits are given normalized to :math:`\psi_{wall}`.
    tol : float, optional
    ...
    # Shift the theta scanning region to the left (padding the end) in
    # order to scan the entire area of interest without finding
    # duplicate (essentially the same) candidates at -π and π
    theta_shift = abs(np.diff(theta_lim) / theta_grid_density)

    theta_min, theta_max = (
        theta_lim[0] + theta_shift,
        theta_lim[1] + theta_shift,
    )
    ...
    fixed_point_candidates = [
        (theta_grid[i, j], psi_grid[i, j]) for i, j in filtered_minima
    ]

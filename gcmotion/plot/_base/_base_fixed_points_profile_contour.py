r"""
Draws fixed points plot. This method is called internally by
``profile_contour()`` as well.
"""

from gcmotion.scripts.fixed_points_bif.fixed_points import (
    fixed_points as get_fixed_points,
)
...
def _base_fixed_points_plot(
    ...
    **kwargs,
    ...
    r"""
    Function that creates a plot of the GC Hamiltonian's fixed points.
    Most of its arguments will be passed into :py:func:`fixed_points`. This
    function is almost always used in :py:func:`profile_contour`, in order
    ...
    method : str, optional
        ...
        fixed points in :py:func:`single_fixed_point`. Can either be
        "fsolve" (deterministic) or "differential evolution" (stochastic).
        Defaults to "fsolve".
    dist_tol : float
        ...
        fixed points must be below this tolerance for the fixed points to
        be considered the same. Defaults to 1e-3.
    ...
    plot_init_cond : bool, optional
        ...
        :py:func:`fixed_points` will be plotted. Defaults to ``False``.
    ...
    fixed_points_output = get_fixed_points(profile=profile, **kwargs)
    logger.info(
        f"""Calculated fixed points ['NUMagnetic_flux'] for fixed_points_plot
         with Pz={profile.PzetaNU}, mu={profile.muNU} in
         {(time() - start):.1f}s"""
    )
    ...
    logger.info(
        f"Classified fixed points and XPoints={X_points}, OPoints={O_points}"
    )
    ...
    if config.RZ_coords and not isinstance(profile.bfield, LAR):
        ...
        logger.info("Converted fixed points from theta, psi to RZ coords")

    # In RZ coords the x axis is not theta so only if not RZ_coords we set
    # these values for the x axis.
    ...
    ax.scatter(
        ...
        yO,
        marker="o",
        edgecolor=config.O_color,
        facecolors="none",
        s=config.O_size,
    )

    logger.info(
        f"""Plotted fixed points for fixed_points_plot with
         Pz={profile.PzetaNU} and mu={profile.muNU}"""
    )

    if config.fp_plot_init_cond:

        thetas_init, psis_initNU = (
            zip(*initial_conditions) if initial_conditions else ([], [])
        )
        ...
        if config.RZ_coords and not isinstance(profile.bfield, LAR):
            ...
            logger.info(
                "Converted initial conditions from theta, psi to RZ coords"
            )
...
def _get_RZ_coords(
    profile: Profile,
    thetas_fixed: float | list | np.ndarray | tuple,
    psis_fixed: Quantity,
) -> list | np.ndarray:
    r"""Simple function that takes in :math:`\theta`, :math:`\psi` and
    calculates their R, Z coordinates."""
    ...
    logger.info(
        f"Calculated RZ values for base fp plot \n{Rs_fixed=}, {Zs_fixed=}"
    )

"""Simple script that draws the profile energy contour with
the fixed O and X Points on the plot."""
...
def fixed_points_energy_contour(profile: Profile, **kwargs):
    ...
    flux_units : str, optional
        The units of the psi/Ptheta axis. Can be "Magnetic_flux"(same as
        "Tesla * meters^2"), "NUmagnetic_flux" (same as "NUTesla *
        NUmeters^2"), "psi_wall", "NUpsi_wall", or any other pint unit with
        the same dimensionality. Defaults to "Tesla * meters^2".
    ...
    potential : bool, optional
        Whether or not to add the potential Φ term when calculating the
    ...
    if config.show:
        logger.info(
            "--> Energy contour with fixed points successfully plotted."
        )
        plt.show()
    else:
        logger.info(
            "--> Energy contour with fixed points returned without plotting"
        )
        plt.clf()

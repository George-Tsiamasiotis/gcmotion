r"""
Calculates and plots the bifurcation diagram of the fixed points for
multiple profiles with different Pzetas or mus.
"""
...
from gcmotion.plot._base._bif_base._base_thetas_bif_plot import (
    _thetas_bif_plot,
)
...
from gcmotion.plot._base._bif_base._base_tpb_plot import (
...
def bifurcation_plot(profile: Profile, COM_values: list, **kwargs):
    ...
    :meta public:
        ...
        thetalim : list, optional
            Limits of the of the :math:`\theta`, :math:`\psi` search area
            with respect to the :math:`\theta` variable.
        ...
        psilim : list, optional
            ...
            CAUTION-> The limits are given normalized to :math:`\psi_{wall}`.
        method : str, optional
            String that indicates which method will be used to find the
            system's fixed points in :py:func:`single_fixed_point`. Can
            either be "fsolve" (deterministic) or "differential evolution"
            (stochastic). Defaults to "fsolve".
        dist_tol : float, optional
            Tolerance below which two fixed points are not considered
            distinct. The differences between both :math:`\theta` and
            :math:`\psi` of the fixed points must be below this tolerance for
            the fixed points to be considered the same. Defaults to 1e-3.
        fp_ic_scan_tol : float, optional
            Tolerance below which the sum of the squares of the time
            derivatives of the :math:`\theta` and :math:`\psi` variables
            is considered zero. It is passed into :py:func:`fp_ic_scan`.
            Defaults to 5 * 1e-8.
        ic_theta_grid_density : int, optional
            Density of the :math:`\theta`, :math:`\psi` 2D grid to be scanned
            for initial conditiond (fixed points candidates) with respect to
            the :math:`\theta` variable. It is passed into
            :py:func:`fp_ic_scan`. Defaults to 400.
        ic_psi_grid_density : int, optional
            Density of the :math:`\theta`, :math:`\psi` 2D grid to be scanned
            for initial conditiond (fixed points candidates) with respect to
            the :math:`\psi` variable. It is passed into
            :py:func:`fp_ic_scan` Defaults to 400.
        random_fp_init_cond : bool, optional
            Boolean determining weather random initial conditions are to be
            used instead of those provided by :py:func:`fp_ic_scan`.
            Defaults to ``False``.
        fp_info : bool, optional
            Boolean determining weather fixed points' information is to be
            is to be printed in the log. Defaults to ``False``.
        bif_info: bool, optional
            Boolean that determines weather information regarding the
            bifurcation process is to be is to be printed in the log.
            Defaults to ``False``.
        fp_ic_info : bool, optional
            Boolean determing weather information on the initial condition is
            to be is to be printed in the log. Defaults to ``False``.
        plot_energy_bif : bool, optional
            Boolean determining weather the energy of each fixed point of each
            profile (each :math:`P_{\zeta}`) is to be plotted.
            Defaults to ``False``.
        which_COM : str, optional
            Determines with regard to which COM (:math:`\mu` or
            :math:`P_{\zeta}`) will the bifurcation analysis take place.
            Essentially determinies the independent variable on the axis' of
            the bifurcation diagram. Defaults to 'Pzeta'.
        energy_units : str, optional
            String specifying the unit of the calculated fixed points'
            energies. Defaults to ``"NUJoule"``.
        energies_info : bool, optional
            Boolean determining weather information on the fixed points'
            energies is to be is to be printed in the log.
            Defaults to ``True``.
        fp_only_confined : bool, optional
            Boolean determining if the search for :math:`\psi_{fixed}` will
            be conducted only for :math:`\psi` < :math:`\psi_{wall}`
    ...
    logger.info(
        f"""Ran bifurcation script for bifurcation plot with
         N={len(COM_values)} and for
         COM = {config.which_COM} in {(time() - start)/60:.1f} mins"""
    )
    ...
    logger.info(
        f"""Made Xthetas, Othetas fixed bifurcation plot for
         COM = {config.which_COM} for {other_COM[0]} = {other_COM[1]}"""
    )

    # Fixed psis bifurcation diagram
    _psis_bif_plot(
        profile=profile,
        COM_values=COM_values,
        X_psis=X_psis,
    ...
    logger.info(
        f"""Made psis fixed bifurcation plot for
         COM = {config.which_COM} for {other_COM[0]} = {other_COM[1]}"""
    )
    ...
    if config.plot_ndfp:
        ...
        logger.info(
            f"""Made number of fixed points bifurcation plot for
             COM = {config.which_COM} for {other_COM[0]} = {other_COM[1]}"""
        )
    ...
    if config.plot_energy_bif:
        ...
        logger.info(
            f"""Made fixed points' energies bifurcation plot for
             COM = {config.which_COM} for {other_COM[0]} = {other_COM[1]}"""
        )

...
def efield_profile(entity: Tokamak | Profile | Particle, **kwargs):
    ...
    # Plot
    axf.plot(psi / psi_wallNU, el_field, lw=config.linewidths)
    axp.plot(psi / psi_wallNU, el_potential, lw=config.linewidths)

    # Add vertical lines indicating the wall
    if config.span[1] >= 1:
        axf.axvline(x=1, color=config.wall_color, lw=config.wall_linewidth)
        axp.axvline(x=1, color=config.wall_color, lw=config.wall_linewidth)
    ...
    axp.set_ylabel(
        rf"$\Phi(\psi) [{config.potential_units}]$", size=config.labelsize
    )
    axp.grid(True)
    ...
    def fmt_Phi(x, y):
        return (
            f"ψ={x*psi_wallNU.m:.4g}[NU], Φ={y:.4g}[{config.potential_units}]"
        )
